...
struct bcm2835_peripheral gpio = {BCM2835_GPIO_BASE, 0};
struct bcm2835_peripheral bsc0 = {BCM2835_BSC0_BASE, 0};
struct bcm2835_peripheral timer_arm = {BCM2835_TIMER_ARM_BASE, 0};
...
void wait_i2c_done() {
		...
		while((!((BCM2835_BSC0_S) & BCM2835_BSC_S_DONE))) {
			usleep(100);
		}
}
...
void i2c_write(char dev_addr, char reg_addr, char *buf, unsigned short len) {
		...
		BCM2835_BSC0_A = dev_addr;
		BCM2835_BSC0_DLEN = len + 1;	// one byte for the register address, plus the buffer length
		
		BCM2835_BSC0_FIFO = reg_addr;	// start register address
		for(idx=0; idx < len; idx++)
			BCM2835_BSC0_FIFO = buf[idx];
			
		BCM2835_BSC0_S = BCM2835_CLEAR_STATUS; // Reset status bits (see #define)
		BCM2835_BSC0_C = BCM2835_START_WRITE;	// Start Write (see #define)
...
void i2c_read(char dev_addr, char reg_addr, char *buf, unsigned short len) {
		...
		BCM2835_BSC0_DLEN = len;
		BCM2835_BSC0_S = BCM2835_CLEAR_STATUS; // Reset status bits (see #define)
		BCM2835_BSC0_C = BCM2835_START_READ;	// Start Read after clearing FIFO (see #define)

		do {
			// Wait for some data to appear in the FIFO
			while((BCM2835_BSC0_S & BCM2835_BSC_S_TA) && !(BCM2835_BSC0_S & BCM2835_BSC_S_RXD));

			// Consume the FIFO
			while((BCM2835_BSC0_S & BCM2835_BSC_S_RXD) && (bufidx < len)) {
				buf[bufidx++] = BCM2835_BSC0_FIFO;
			}
		} while((!(BCM2835_BSC0_S & BCM2835_BSC_S_DONE)));
}
...
void dump_bsc_status() {
	...
	unsigned int s = BCM2835_BSC0_S;
	
	printf("BSC0_S: ERR=%d  RXF=%d  TXE=%d  RXD=%d  TXD=%d  RXR=%d  TXW=%d  DONE=%d  TA=%d\n",
		(s & BCM2835_BSC_S_ERR) != 0, 
		(s & BCM2835_BSC_S_RXF) != 0, 
		(s & BCM2835_BSC_S_TXE) != 0, 
		(s & BCM2835_BSC_S_RXD) != 0, 
		(s & BCM2835_BSC_S_TXD) != 0, 
		(s & BCM2835_BSC_S_RXR) != 0, 
		(s & BCM2835_BSC_S_TXW) != 0, 
		(s & BCM2835_BSC_S_DONE) != 0, 
		(s & BCM2835_BSC_S_TA) != 0 );
}
